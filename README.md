...
Players who do not use the `can you` format can use:

```text
/iristask
```

The slash command asks for:

- assignee: Olive, Eli / Elithe, or Admin
- task: the request text

Iris still logs the task privately and gives the same simple acknowledgement.
...
Olive and Eli can also be admins. Iris still treats them as Olive/Eli for personal task lists and praise because their configured user IDs are checked before generic admin handling. `Eli` and `Elithe` both assign to the same configured `ELITHE_USER_ID`.
...
- Normal message-based task creation
- `/iristask` fallback for players who do not follow the message format
...
When generating the invite URL, use these OAuth2 scopes:
...
`applications.commands` is needed for `/iristask`.
