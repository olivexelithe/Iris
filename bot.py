...
TASK_PATTERN = re.compile(
    r"^\s*(?:<@!?(?P<mention>\d+)>\s*|<@&(?P<rolemention>\d+)>\s*|@(?P<atname>olive|elithe|eli|cookie|admin)\s*|(?P<name>olive|elithe|eli|cookie|admin)\s*)"
...
COOKIE_USER_ID = 592819003752448010
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        cookie_user_id = _optional_int("COOKIE_USER_ID", COOKIE_USER_ID)
...
def assignee_from_key(assignee_key: str, config: Config) -> Optional[tuple[str, Optional[int]]]:
    ...
    elif assignee_key == "cookie":
        return "Cookie", config.cookie_user_id
...
def user_id_to_assignee(user_id: Optional[str], config: Config) -> Optional[str]:
    ...
    if parsed == config.elithe_user_id:
        return "elithe"
    if parsed == config.cookie_user_id:
        return "cookie"
    return None
...
def is_task_team_member(member: discord.Member, config: Config) -> bool:
    return member.id in {config.olive_user_id, config.elithe_user_id, config.cookie_user_id} or has_admin_role(member, config)
...
def task_team_name(member: discord.Member, config: Config) -> str:
    ...
    if member.id == config.elithe_user_id:
        return "Eli"
    if member.id == config.cookie_user_id:
        return "Cookie"
...
def completion_praise(user: discord.User | discord.Member, task_id: str, config: Config) -> str:
    ...
    if user.id == config.cookie_user_id:
        template = random.choice(COMPLETION_PRAISES)
        return template.format(name="Cookie", task_id=task_id)
...
class IrisSlashCommands(commands.Cog):
    ...
    @app_commands.choices(
        assignee=[
            ...
            app_commands.Choice(name="Cookie", value="cookie"),
