...
def parse_task_message(content: str, config: Config) -> Optional[tuple[str, Optional[int], str]]:
    ...
    if assignee_key == "olive":
        assignee_id = config.olive_user_id
        assignee_label = "Olive"
    elif assignee_key in {"elithe", "eli"}:
        assignee_id = config.elithe_user_id
        assignee_label = "Eli"
...
class TaskBot(commands.Bot):
    ...
    @tasks.loop(hours=1)
    async def hourly_reminders(self) -> None:
    ...
    @hourly_reminders.before_loop
    async def before_hourly_reminders(self) -> None:
        await self.wait_until_ready()
